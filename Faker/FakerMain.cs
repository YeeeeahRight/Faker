...
namespace Faker
{
    public class FakerMain: IFaker
    {
        ...
        public FakerMain(FakerConfiguration Configur)
        {
        ...
        public object Create(Type type) 
        {
            ...
            FakerMain FakerMain = new FakerMain(Configuration);
            int seed = (int)DateTime.Now.Ticks & 0x0000FFFF;
            GeneratorContext Context = new GeneratorContext(new Random(seed),type, FakerMain);


            IValueGenerator generator = FindGeneratorByType(type);
                ...
                foreach (FieldInfo field in Fields)
                {
                    if (IsValueSet(field,obj))
                    {
                        ...
                        else
                        {
                            int seed = (int)DateTime.Now.Ticks & 0x0000FFFF;
                            FakerMain FakerMain = new FakerMain(Configuration);
                            GeneratorContext Context = new GeneratorContext(new Random(seed), field.FieldType, FakerMain);
                ...
                foreach (PropertyInfo property in Properties)
                {
                    if ((property.CanWrite)&& (IsValueSet(property, obj)))
                    {
                        ...
                        else
                        {
                            int seed = (int)DateTime.Now.Ticks & 0x0000FFFF;
                            FakerMain FakerMain = new FakerMain(Configuration);
                            GeneratorContext Context = new GeneratorContext(new Random(seed), property.PropertyType, FakerMain);
                            IValueGenerator test = (IValueGenerator)Activator.CreateInstance(configurationRule.GeneratorName);
        ...
        public IValueGenerator FindGeneratorByType(Type type)
        {
